...
public class PageBar {
	...
	public PageBar(HttpServletRequest request, int totalData, int pageBarSize, String location) {
		...
		if (pageNo == 1) {
			pageBar += "<span>&nbsp[?ù¥?†Ñ]&nbsp</span>";
		} else {
		...
		if (pageNo > totalPage) {
			pageBar += "<span>&nbsp[?ã§?ùå]&nbsp</span>";
		} else {
			pageBar += "<a href='" + request.getContextPath() + location + "?cPage=" + pageNo + "'>&nbsp[?ã§?ùå]&nbsp</a>";
		}
	...
	public PageBar(HttpServletRequest request, int totalData, int pageBarSize, String location, String query) {
		...
		if (pageNo > totalPage) {
			...
			pageBar += "<a href='" + request.getContextPath() + location + "?"+query+"&cPage=" + pageNo + "'>&nbsp[?ã§?ùå]&nbsp</a>";
		}
